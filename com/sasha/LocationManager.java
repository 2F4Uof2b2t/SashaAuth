...
public class LocationManager {
    ...
    public static void storeLogOffLocation(SecurePlayer sp) throws IOException {
        File file = new File("SashaAuth_" + sp.getPlayer().getName() + ".txt");
        if (!file.exists()) {
            file.createNewFile();
        }
    ...
    public static void storeLogOffLocation(Player sp) throws IOException {
        File file = new File("SashaAuth_" + sp.getName() + ".txt");
        if (!file.exists()) {
            file.createNewFile();
        }
        Writer w = new FileWriter(file, false);
    ...
    public static Location getLocation(Player sp) throws IOException {
        ...
        return new Location(Bukkit.getWorld(locArray[0]), Double.parseDouble(locArray[1]), Double.parseDouble(locArray[2]), Double.parseDouble(locArray[3]));
    }
}
